...
* memoryless full node. They download and verify blocks, but only store the headers. Can be a mining pool.
...
The big drawback of the memoryless full node strategy is that every block needs to include cryptographic proofs of all the consensus state needed for processing that block. For example, if you want to make a tx to spend some of your money, the block containing this tx needs to include a cryptographic proof of your balance. That way the full nodes can be sure that you have sufficient money to afford to spend the amount that you want to spend.
...
A memoryless full node needs about 3 merkle proofs per tx. The size of the proof is log2(100 billion). So around 7.5 kilobytes per tx.

432/7.5 is about 60. So a memoryless full node that stores all of the consensus state, is around 60 more efficient than a full node that stores the consensus state. Where efficiency is in terms of total time spent reading/writing to hard drives per tx.

Memoryless full nodes that don't store the blocks can be far cheaper than archival nodes. Non-archival memoryless full nodes never need to read or write to a hard drive.

Comparison of archival full nodes
============

Memoryless full node blockchains, their txs are about 7.5 kilobytes each. In comparison to around 400 bytes for blockchains that store the consensus state. That means the blocks are around 40x bigger for memoryless full nodes.
...
So full nodes with memory, the archival nodes would need around 400 terabytes of hard drive space.
A blockchain with memoryless full nodes, it would need 7500 terabytes of hard drive space per archival node.

The cost of archival nodes is significantly higher in the memoryless design, but it is expected that archival nodes will be a minority of the network, so the total cost of a network using the memoryless full node design can be lower.

Comparison of mining pools
============

In the memoryless full node design, a mining pool can be a memoryless full node.
But in the full node with memory design, a mining pool needs to be a consensus state full node, which is far more expensive.
...
Memoryless full nodes for mining in comparison need around 1 gigabyte of RAM to be able to verify the blocks, which costs around $10.

Comparison of consensus state full nodes
================

In the memoryless full node strategy, a consensus state full node can store whatever fraction of the consensus state they are interested in. So this is a kind of sharding. We can divide the consensus state onto different nodes, and the memory requirement for any individual node can be limited.

If we don't use the memoryless full node strategy, then every cosnensus state full node needs to store the entire consensus state, because that is the only way to verify the blocks. For a global blockchain, that means a 10 terabyte hard drive.

Verifying Txs in Parallel in memoryless full nodes
============
...
Verifying Smart Contracts in Parallel in memoryless full nodes
==========
