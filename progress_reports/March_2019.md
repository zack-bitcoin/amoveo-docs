...
An attempt was made oracle list
                 C  S  L  M  O  B  C  F   E    total
Amoveo           10 8  10 10 10 10 7  10  10   85
Group Gnosis     1  0  5  0  1  10 10 5   3    35
Augur            1  0  5  0  1  10 9  -5  3    23
Bitcoin Hivemind 1  0  0  0  1  5  2  -5  5    8
...
(learn about the different levels of trust here.)[../basics/trust_theory.md]
Some teams are using trust level 3.1 oracles.
...
Stox has a collateral which is given by the person who creates the oracle. The amount of collateral does not change, the volume of bets is limited by how much collateral is given by the oracle creator. Stox cannot escalate. level 3.1 security oracles.

Aeternity, chainlink, Komodo, and Bodhi use trusted feeds with no collateral. These are vulnerable to bribery and retirement attacks. level 4.1 security oracles.

Augur has collateral in the form of rep, a subcurrency. The amount of collateral is the market cap of rep. The volume of bets happening in all markets secured by the Augur oracle is limited by the market cap of the Augur oracle, including off-chain bets in the channels. Augur cannot escalate. level 3.1 security oracles.

Hivemind works similarly as Augur, they have collateral called "votecoins", a subcurrency. level 3.1 security oracles.

Gnosis gives an explanation of their oracle with escalation [here](https://blog.gnosis.pm/a-visit-to-the-oracle-fefc9dec5462). the "Ultimate Oracle" is the part with escalation. It is implemented in solidity [here](https://github.com/gnosis/gnosis-contracts/tree/master/contracts/Oracles) level 3.1 security oracles.

Zen protocol has level 4.1 security oracles.

The Amoveo oracle has escalation. It is tested [here](/tests/market.py), edge cases of the oracle are tested [here](/apps/amoveo_core/src/consensus/txs/test_txs.erl) level 1.1 security oracles
The Amoveo oracle is live on the Amoveo testnet.

Every team which lacks oracles of level 3.1 or greater will be moved to a seperate list, as they have no possibility of success. It is just a facade of a project.
